...
for k in range(steps):
    ...
    if random()<0.5:
        ...
        if  i<19 and j<19:
            ...
            dE = n[i,j]*(n[i,j+1]+ n[-1,j]+ n[i,j-1]+ n[i-1,j])

        elif j==19 and i<19:
            dE = n[i,j]*(n[i,-1]+ n[i+1,j]+ n[i,j-1]+ n[i-1,j])
            
        elif j==19 and i==19:
            dE = n[i,j]* (n[i,-1]+ n[-1,j]+ n[i,j-1]+ n[i-1,j])
...
plt.savefig("/Users/petra/OneDrive/Documents/PHYS304/pbudavari_hw/"+ str("MCS_a") + ".png")
plt.close()
